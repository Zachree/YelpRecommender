...
public class HashTable implements Serializable {
    ...
    static class Node implements Serializable {
        ...
        Node(Object k, Object v, double lat, double lng){
            key = k;
            val = v;
    		latitude = lat;
    		longitude = lng;
    ...
    Object get(Object key){
        ...
        return null;					   // fall through and return nothing if matching key was not found
    }
    
    double getLatitude(Object key){
        int h = MurmurHash2.hash32(key.toString());
        int i = h & (table.length-1);
        /*for(Node e = table[i]; e != null; e = e.next) {
            if(key.equals(e.key))
    ...
    double getLongitude(Object key){
        int h = MurmurHash2.hash32(key.toString());
        int i = h & (table.length-1);
        /*for(Node e = table[i]; e != null; e = e.next) {
            if(key.equals(e.key))
    ...
    void put(Object key, Object val) {
        ...
        if(table[i] != null){               // if collision,
            if(key.equals(table[i].key)){   // and same key,
                table[i].val += " " + val;  // append current value to pre-existing value. (for the purpose of keeping different reviews of the same business in one slot)
                return;
            }
            else                           // or if not same key,
                i = LinearProbe(i, key);   // linear probe whole table for matching key, but if none, return next open slot.
        }
        if(table[i] == null)               // if LinearProbe returned an empty slot (or slot was empty to begin with),
    ...
    void put(Object key, Object val, double lat, double lng) {
        int h = MurmurHash2.hash32(key.toString());
        int i = h & (table.length-1);
        /*for(Node e = table[i]; e != null; e = e.next){
            if(key.equals(e.key)){
        ...
        if(table[i] == null)               // if LinearProbe returned an empty slot (or slot was empty to begin with),
            table[i] = new Node(key, val, lat, lng);
        else{                              // or if LinearProbe returned a slot with a matching key.
            table[i].val += " " + val;
            return;
        }
...
//        }
        for(int i = 0; i < table.length; ++i){
            if(oldTable[i] != null){
                ...
                newTable[j] = new Node(oldTable[i].key, oldTable[i].val, oldTable[i].latitude, oldTable[i].longitude);
            }
        }
        table = newTable;
    }
...
//        }
	...
	public static void diskWrite(Node[] table) {
	       try {
	           FileOutputStream file = new FileOutputStream("hashtable.ser");
